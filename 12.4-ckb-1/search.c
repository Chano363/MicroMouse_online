...
int shouldContinueSearch(void) {
    // 0. 步数限制（防止死循环）
    static int continueSteps = 0;
    continueSteps++;
    if (continueSteps > GucMaxContinueSteps) {
        printf("达到最大继续搜索步数(%d)，停止搜索\n", GucMaxContinueSteps);
        continueSteps = 0;  // 重置
        return 0;
    }
    ...
    if (GucSearchCoverage > 0.90f) { // 降低到90%
        printf("覆盖率已达到%.1f%%，停止搜索\n", GucSearchCoverage * 100);
        continueSteps = 0;
        return 0;
    }
    ...
    if (!findCriticalUnexplored(&testTarget)) {
        printf("没有找到关键未探索区域，停止搜索\n");
        continueSteps = 0;
        return 0;
    }

    // 3. 检查未探索区域的质量（更宽松的条件）
    int meaningfulUnexplored = 0;

    // 只检查距离鼠标10步内的未探索区域
    for (int i = 0; i < MAZETYPE; i++) {
        for (int j = 0; j < MAZETYPE; j++) {
            if (GucMapGet[i][j] == 0) {
                int distToMouse = abs(i - GmcMouse.cX) + abs(j - GmcMouse.cY);
                if (distToMouse <= 10) {  // 只关心附近的区域
                    meaningfulUnexplored++;
                }
            }
        }
    }

    if (meaningfulUnexplored == 0) {
        printf("附近没有有意义的未探索区域，停止搜索\n");
        continueSteps = 0;
        return 0;
    }

    // 4. 检查探索效率（如果最近几步覆盖率没提升）
    static float lastCoverage = 0;
    static int noProgressSteps = 0;

    if (GucSearchCoverage - lastCoverage < 0.005f) {  // 覆盖率提升小于0.5%
        noProgressSteps++;
    }
    else {
        noProgressSteps = 0;
    }
    lastCoverage = GucSearchCoverage;

    if (noProgressSteps > 10) {  // 连续10步没有进展
        printf("连续%d步覆盖率无显著提升，停止搜索\n", noProgressSteps);
        continueSteps = 0;
        noProgressSteps = 0;
        return 0;
    }
