...
void continueSearchMethod(void) {
    ...
    if (findCriticalUnexplored(&target) && (rand() % 100 < 50)) {
        ...
        if (tw_Search(GmcMouse.cX, GmcMouse.cY, target.cX, target.cY,
            path, &pathLength)) {
...
void smartFloodFill(void) {
    ...
    mapStepSearch(keyArea.cX, keyArea.cY);
...
int isCriticalPathExplored(void) {
    ...
    // 检查到起点的路径
    if (tw_Search(GmcMouse.cX, GmcMouse.cY, GucXStart, GucYStart, path1, &len1)) {
    ...
    if (tw_Search(GmcMouse.cX, GmcMouse.cY, GoalX, GoalY, path2, &len2)) {
