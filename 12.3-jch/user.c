...
int userMain(void)
{
	...
	while (1)
	{
		...
		case MAZESEARCH:
			...
			if (canMoveNum > 0) {
				if (use_tw()) {
					...
					floodSerchway();      // 正常洪水填充（不是smartFloodFill）
				}
			}
			else {
				smartBacktrack();          // 智能回溯
		...
		case SPURT:
			...
			if (tw_Search(GmcMouse.cX, GmcMouse.cY, GucXStart, GucYStart,
			...
			if (GmcMouse.cX == GucXStart && GmcMouse.cY == GucYStart) {
				...
				if (tw_Search(GmcMouse.cX, GmcMouse.cY, GoalX, GoalY,
...
// 优化后的洪水填充（使用队列）
// 更高效的洪水填充算法
void mapStepSearch(char cX, char cY)
{
