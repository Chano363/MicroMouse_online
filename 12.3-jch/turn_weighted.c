...
// 计算转向惩罚
int calculateTurnPenalty(int parentX, int parentY, int currentX, int currentY, int nextX, int nextY) {
    // 如果没有父节点（起点），没有转向惩罚
...
int tw_Search(int startX, int startY, int goalX, int goalY, MAZECOOR* path, int* pathLength) {
    twNode nodes[MAZETYPE][MAZETYPE];
    ...
    while (!pq_empty(&openList)) {
        ...
        for (int i = 0; i < neighborCount; i++) {
            ...
            // 计算转向惩罚
            int turnPenalty = calculateTurnPenalty(
            ...
            // 计算 tentative_g（从起点经过当前节点到邻居的代价）
        ...
        while (i < *pathLength - 2) {
            ...
            if (x1 == x3) {
                ...
                for (int y = startY; y < endY; y++) {
                    if (!(GucMapBlock[x1][y] & 0x01)) { // 检查向上的墙壁
                        hasDirectPath = 0;
                        break;
                    }
                }
                ...
                for (int x = startX; x < endX; x++) {
                    if (!(GucMapBlock[x][y1] & 0x02)) { // 检查向右的墙壁
                        hasDirectPath = 0;
                        break;
                    }
                }
...
void tw_method(void) {
    ...
    if (tw_Search(GmcMouse.cX, GmcMouse.cY, targetX, targetY, path, &pathLength)) {
        // 路径平滑处理
