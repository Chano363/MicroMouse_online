...
void smoothPath(MAZECOOR* path, int* pathLength) {
	...
	while (i < *pathLength - 2) {
		...
		int x2 = path[i + 2].cX, y2 = path[i + 2].cY;

		// 检查是否可以直线到达（没有墙壁阻挡）
		if (isDirectPath(x1, y1, x2, y2)) {
			// 移除中间点
...
int isDirectPath(int x1, int y1, int x2, int y2) {
	if (x1 == x2) {
		// 垂直方向
		int minY = (y1 < y2) ? y1 : y2;
		int maxY = (y1 > y2) ? y1 : y2;
		for (int y = minY; y < maxY; y++) {
			if (!(GucMapBlock[x1][y] & 0x01)) return 0; // 检查上墙
		}
		return 1;
	}
	else if (y1 == y2) {
		...
		for (int x = minX; x < maxX; x++) {
			if (!(GucMapBlock[x][y1] & 0x02)) return 0; // 检查右墙
		}
		return 1;
	}
	return 0;
